...
export default function CompetitionDetail({ lang, slug }: CompetitionDetailProps) {
  ...
  const t = {
    pt: { future: "Partidas Futuras", past: "Partidas Anteriores", futuresBtn: "Futuras", pastBtn: "Anteriores", back: "← Voltar", noMatches: "Nenhuma partida encontrada", page: "Página", of: "de", prev: "Anterior", next: "Próxima", loadingMatches: "Carregando partidas...", loadingComp: "Carregando detalhes do campeonato..." },
    en: { future: "Upcoming Matches", past: "Past Matches", futuresBtn: "Upcoming", pastBtn: "Past", back: "← Back", noMatches: "No matches found", page: "Page", of: "of", prev: "Previous", next: "Next", loadingMatches: "Loading matches...", loadingComp: "Loading competition details..." },
    es: { future: "Partidos Futuros", past: "Partidos Anteriores", futuresBtn: "Futuros", pastBtn: "Anteriores", back: "← Volver", noMatches: "No se encontraron partidos", page: "Página", of: "de", prev: "Anterior", next: "Próxima", loadingMatches: "Cargando partidos...", loadingComp: "Cargando detalles de la competición..." },
    fr: { future: "Matchs à venir", past: "Matchs passés", futuresBtn: "À venir", pastBtn: "Passés", back: "← Retour", noMatches: "Aucun match trouvé", page: "Page", of: "sur", prev: "Précédent", next: "Suivant", loadingMatches: "Chargement des matchs...", loadingComp: "Chargement des détails de la compétition..." },
  }[currentLang] || { future: "Partidas Futuras", past: "Partidas Anteriores", futuresBtn: "Futuras", pastBtn: "Anteriores", back: "← Voltar", noMatches: "Nenhuma partida encontrada", page: "Página", of: "de", prev: "Anterior", next: "Próxima", loadingMatches: "Carregando partidas...", loadingComp: "Carregando detalhes do campeonato..." };
  ...
  useEffect(() => {
    const fetchCompetitionData = async () => {
      ...
      if (error || !data) {
        console.error('Campeonato não encontrado:', error);
        router.push(`/${lang}/competition`);
        return;
      }
  ...
  const fetchMatchesByCompetition = useCallback(async (compId: string, page: number) => {
  ...
  useEffect(() => {
    if (competition) {
      fetchMatchesByCompetition(competition.id, currentPage);
    }
  }, [competition, currentPage, showPast, fetchMatchesByCompetition]);
  ...
  if (loadingComp) {
    return <p className="text-center py-20 text-zinc-400 bg-zinc-950 min-h-screen">{t.loadingComp}</p>;
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 bg-zinc-950 min-h-screen text-white">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
        <div className="flex flex-col sm:flex-row items-center text-center sm:text-left gap-4">
          ...
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">{getName(competition)}</h2>
        ...
        <div className="grid grid-cols-3 sm:flex sm:flex-wrap gap-2 sm:gap-3 w-full lg:w-auto">
          <button 
            ...
            className={`px-2 sm:px-6 py-3 text-xs sm:text-sm font-medium rounded-2xl transition-colors whitespace-nowrap ${
              !showPast ? 'bg-emerald-600 text-white' : 'bg-zinc-800 hover:bg-zinc-700 text-zinc-300'
            }`}
          >
            {t.futuresBtn}
          </button>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row justify-center items-center gap-3 mt-10 sm:mt-12 w-full">
          <button 
            ...
            className="w-full sm:w-auto text-sm font-medium px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-2xl disabled:opacity-40 disabled:hover:bg-zinc-800 transition-colors order-2 sm:order-1"
          >
            {t.prev}
          </button>
          
          <span className="w-full sm:w-auto text-center text-xs sm:text-sm px-6 py-3 bg-zinc-900 border border-zinc-800 text-zinc-300 rounded-2xl order-1 sm:order-2">
          ...
          <button 
            ...
            className="w-full sm:w-auto text-sm font-medium px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-2xl disabled:opacity-40 disabled:hover:bg-zinc-800 transition-colors order-3"
          >
            {t.next}
          </button>
        </div>
