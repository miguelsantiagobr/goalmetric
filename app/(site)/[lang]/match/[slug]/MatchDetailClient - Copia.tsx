...
export default function MatchDetailClient({ match, lang, currentLang }: MatchDetailClientProps) {
  ...
  const faqTranslations = {
    ...
    es: {
      q1: `¿Cuál es la mejor apuesta para ${homeName} x ${awayName}?`,
      a1: getText(match, 'best_bet') 
        ? `La mejor apuesta es ${getText(match, 'best_bet')}.` 
  ...
  const t = {
    ...
    es: { 
      ...
      titulologinpremium: "Análisis de Experto", facalogin: "Inicia sesión para desbloquear la apuesta recomendada, la predicción exacta de goles y el jugador con mayor probabilidad de éxito.",
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <nav className="text-xs lg:text-sm text-zinc-500 mb-8 flex items-center gap-2 flex-wrap">
          ...
          <span className="text-zinc-300 font-medium truncate max-w-[200px] sm:max-w-none">
            {getText(match.home_team, 'name')} vs {getText(match.away_team, 'name')}
          </span>
        </nav>
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "itemListElement": [
                ...
                { "@type": "ListItem", "@position": 3, "name": `${getText(match.home_team, 'name')} vs ${getText(match.away_team, 'name')}`, "item": `https://goalmetric.online/${lang}/match/${match.slug || ''}` }
              ]
            })
          }}
        />
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-emerald-400 font-semibold text-xl tracking-wide uppercase">
            {getText(match.home_team, 'name')} x {getText(match.away_team, 'name')}<br/>
        ...
        <div className="flex items-center justify-between max-w-5xl mx-auto mb-12">
          ...
          <div className="flex-1 text-center group">
            {match.home_team?.logo_url && (
              <div className="relative mx-auto w-32 h-32 flex items-center justify-center mb-6 transition-transform duration-500 group-hover:scale-110">
                ...
                <img src={match.home_team.logo_url} className="relative z-10 w-28 h-28 object-contain drop-shadow-2xl" alt={`Logo ${getText(match.home_team, 'name')}`} />
              </div>
            )}
            <p className="text-2xl sm:text-3xl font-bold text-white tracking-tight">{getText(match.home_team, 'name')}</p>
          </div>

          {/* VS + Placar */}
          <div className="text-center px-4 sm:px-10">
            <div className="inline-flex items-center justify-center mb-4">
              <span className="text-2xl sm:text-3xl font-black text-zinc-700 tracking-tighter">VS</span>
            </div>
            {match.predicted_score && (
              <>
                <p className="text-xs sm:text-sm font-medium text-emerald-400/80 mb-1 tracking-widest uppercase">{t.previsaoplacar}</p>
                <p className="text-5xl sm:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-500 tracking-[0.09em] drop-shadow-md">
                  {match.predicted_score}
                </p>
              </>
            )}
          ...
          <div className="flex-1 text-center group">
            {match.away_team?.logo_url && (
              <div className="relative mx-auto w-32 h-32 flex items-center justify-center mb-6 transition-transform duration-500 group-hover:scale-110">
                <div className="absolute inset-0 bg-emerald-500/10 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <img src={match.away_team.logo_url} alt={`Logo ${getText(match.away_team, 'name')}`} className="relative z-10 w-28 h-28 object-contain drop-shadow-2xl" />
              </div>
            )}
            <p className="text-2xl sm:text-3xl font-bold text-white tracking-tight">{getText(match.away_team, 'name')}</p>
          </div>
        </div>
        ...
        <div className="relative rounded-[2.5rem] mb-14 overflow-hidden border border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.5)]">
          ...
          {/* Overlay Premium da Melhor Aposta */}
          {!isPremiumVisible && (
            <div 
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {enrichedOdds.map((odd: any, index: number) => (
                <a
                  key={index} href={odd.link} target="_blank" rel="noopener noreferrer"
        ...
        {/* ==================== ESTATÍSTICAS & GOLS ESPERADOS (Bloqueio B) ==================== */}
        <div className="mb-16">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-6">
            ...
            {/* Previsão de Gols / Gols Esperados (BLOQUEADO) */}
...
<span className="text-white text-[10px] font-bold uppercase tracking-widest bg-emerald-500/20 px-3 py-1 rounded-full">{t.avisopremium}</span>
        ...
        {match.key_player && (
          <div className="relative mb-16 rounded-[2.5rem] overflow-hidden border border-white/5 shadow-2xl">
            ...
            {/* Overlay Premium do Jogador Destaque */}
            {!isPremiumVisible && (
              <div 
                onClick={handleLockedClick}
                className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-zinc-950/70 backdrop-blur-[6px] cursor-pointer hover:bg-zinc-950/80 transition-all duration-300"
              >
